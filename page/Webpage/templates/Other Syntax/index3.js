...
var earnings = d3.json(earningsurl).then(function(earningsdata){
  ...
  const earningsdate = Object.keys(earningsseries).reverse().map(timestamp => {
   ...
   //average of earnings
   let sum = earningseps.filter((x) => x.length).reduce((a, b) => a + b.length, 0);
   let count = earningseps.filter((x) => x.length).length;
   let avg = sum/count;
...
//average function
let sum = volseries.filter((x) => x.length).reduce((a, b) => a + b.length, 0);
